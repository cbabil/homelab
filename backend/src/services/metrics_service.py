...
from models.metrics import ContainerMetrics, ServerMetrics
...
class MetricsService:
    ...
    def _parse_cpu_percent(self, output: str) -> float:
        ...
        try:
            ...
            match = re.search(
                r"(\d+\.?\d*)\s*(?:%\s*)?us.*?(\d+\.?\d*)\s*(?:%\s*)?sy", output
            )
            if match:
    ...
    async def collect_server_metrics(self, server_id: str) -> ServerMetrics | None:
        ...
        try:
            ...
            exit_code, mem_out, _ = await self.ssh_service.execute_command(
                server_id, "free -k | grep -E '^Mem:'"
            )
            mem_used, mem_total, mem_percent = (
                self._parse_memory(mem_out) if exit_code == 0 else (0, 0, 0.0)
            )

            # Collect Disk
            exit_code, disk_out, _ = await self.ssh_service.execute_command(
                server_id, "df -h / | tail -1"
            )
            disk_used, disk_total, disk_percent = (
                self._parse_disk(disk_out) if exit_code == 0 else (0, 0, 0.0)
            )
    ...
    async def collect_container_metrics(self, server_id: str) -> list[ContainerMetrics]:
        ...
        try:
            ...
            exit_code, output, _ = await self.ssh_service.execute_command(
                server_id, cmd
            )
    ...
    async def get_server_metrics(
        ...
        try:
            ...
            return await self.db_service.get_server_metrics(
                server_id=server_id, since=since.isoformat()
            )
        except Exception as e:
    ...
    async def get_container_metrics(
        self, server_id: str, container_name: str = None, period: str = "24h"
