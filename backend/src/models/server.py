...
class SystemInfo(BaseModel):
    ...
    docker_version: str | None = Field(None, description="Docker version if installed")
    agent_status: str | None = Field(
        None, description="Agent container status (running/not running)"
    )
    agent_version: str | None = Field(None, description="Agent version if installed")
...
class ServerConnection(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Server display name"
    )
    host: str = Field(
        ..., min_length=1, max_length=255, description="Server hostname or IP"
    )
    ...
    system_info: SystemInfo | None = Field(None, description="System information")
    docker_installed: bool = Field(
        default=False, description="Whether Docker is installed"
    )
    system_info_updated_at: str | None = Field(
        None, description="When system info was last updated"
    )
