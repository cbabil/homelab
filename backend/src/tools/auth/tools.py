...
class AuthTools:
    ...
    async def logout(self, session_id: str = None, username: str = None, ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if actual_username == "unknown" and session_id:
                try:
                    db_session = await self.auth_service.session_service.get_session(session_id)
                    if db_session:
                        ...
                        if user_id:
                            ...
                            if user:
                                ...
                                logger.info("Username retrieved from database session",
                                          session_id=session_id, username=actual_username)
                except Exception as e:
                    logger.warning("Could not retrieve username from database session",
                                 session_id=session_id, error=str(e))

            # 3. From legacy in-memory session (fallback)
            if actual_username == "unknown" and session_id and session_id in self.auth_service.sessions:
            ...
            if session_id:
                try:
                    terminated_count = await self.auth_service.session_service.delete_session(
                        session_id=session_id,
                        terminated_by=actual_username if actual_username != "unknown" else "system"
                    )
                    if terminated_count > 0:
                        logger.info("Database session terminated", session_id=session_id, username=actual_username)
                except Exception as e:
                    logger.warning("Failed to terminate database session", session_id=session_id, error=str(e))
            ...
            if session_id and session_id in self.auth_service.sessions:
                ...
                logger.info("In-memory session invalidated", session_id=session_id, username=actual_username)
            elif session_id:
                logger.debug("Session not found in memory for logout", session_id=session_id, username=actual_username)
    ...
    async def create_initial_admin(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_setup:
                logger.warning("Attempted to create initial admin when system is already set up")
                return {
                    "success": False,
                    "error": "ALREADY_SETUP",
            ...
            if await self.auth_service.has_admin_user():
                logger.warning("Attempted to create initial admin when one already exists")
                return {
                    "success": False,
                    "error": "ADMIN_EXISTS",
            ...
            setup_marked = await self.auth_service.db_service.mark_system_setup_complete(user.id)
            if not setup_marked:
                logger.warning("Failed to mark system as setup complete", user_id=user.id)
                # Don't fail the whole operation - admin was created successfully

            logger.info("Initial admin user created and system marked as setup",
    ...
    async def get_user_by_username(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to get user by username", username=params.get("username"), error=str(e))
            return {
                "success": False,
                "message": f"Failed to get user: {str(e)}",
                "error": "GET_USER_ERROR"
    ...
    async def reset_user_password(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        """Reset a user's password.

        This is an admin-level operation for CLI use.
        ...
        try:
            ...
            logger.error("Failed to reset password", username=params.get("username"), error=str(e))
            return {
                "success": False,
    ...
    # Rate limiting: track failed password change attempts per user
    _password_change_attempts: Dict[str, Dict[str, Any]] = {}
    _MAX_ATTEMPTS = 5
    _LOCKOUT_MINUTES = 15

    async def change_password(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not payload:
                logger.warning("Password change attempt with invalid token", client_ip=client_ip)
                return {
                    "success": False,
            ...
            if not user or not user.is_active:
                logger.warning("Password change attempt for invalid/inactive user", user_id=user_id, client_ip=client_ip)
                return {
                    "success": False,
            ...
            if attempt_key in self._password_change_attempts:
                ...
                if lockout_until and now < lockout_until:
                    ...
                    logger.warning("Password change blocked - account locked",
                                 username=username, client_ip=client_ip, remaining_minutes=remaining)
                    await self.auth_service._log_security_event(
                        "PASSWORD_CHANGE_BLOCKED", username, False,
            ...
            stored_hash = await self.auth_service.db_service.get_user_password_hash(username)
            if not stored_hash:
            ...
            if not verify_password(current_password, stored_hash):
                ...
                if attempt_key not in self._password_change_attempts:
                    self._password_change_attempts[attempt_key] = {"count": 0, "first_attempt": now}
                ...
                if attempt_count >= self._MAX_ATTEMPTS:
                    self._password_change_attempts[attempt_key]["lockout_until"] = now + timedelta(minutes=self._LOCKOUT_MINUTES)
                    logger.warning("Password change lockout triggered",
                                 username=username, client_ip=client_ip, attempts=attempt_count)

                await self.auth_service._log_security_event(
                    "PASSWORD_CHANGE_FAILED", username, False,
                    client_ip=client_ip, user_agent=user_agent
                )
            ...
            success = await self.auth_service.db_service.update_user_password(username, new_hash)
            ...
            await self.auth_service._log_security_event(
                "PASSWORD_CHANGED", username, True,
                client_ip=client_ip, user_agent=user_agent
            )

            logger.info("Password changed successfully", username=username, client_ip=client_ip)
            ...
            logger.error("Password change error", username=username, error=str(e), error_type=type(e).__name__)

            # Log failed attempt
            try:
                await self.auth_service._log_security_event(
                    "PASSWORD_CHANGE_ERROR", username, False,
                    client_ip=client_ip, user_agent=user_agent
                )
            except Exception:
                pass
    ...
    async def update_avatar(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if avatar:
                ...
                allowed_types = ["data:image/png;base64,", "data:image/jpeg;base64,",
            ...
            success = await self.auth_service.db_service.update_user_avatar(user_id, avatar_to_save)
            ...
            logger.info("Avatar updated successfully", user_id=user_id, has_avatar=avatar_to_save is not None)

            return {
                "success": True,
                "message": "Avatar updated successfully" if avatar_to_save else "Avatar removed successfully"
            }

        except Exception as e:
            logger.error("Avatar update error", error=str(e), error_type=type(e).__name__)
            return {
                "success": False,
    ...
    async def get_locked_accounts(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user.role.value != "admin":
                logger.warning("Non-admin user attempted to get locked accounts",
            ...
            if identifier:
                ...
                is_locked, lock_info = await self.auth_service.db_service.is_account_locked(
            ...
            logger.info("Retrieved locked accounts", count=len(locked_accounts), admin=user.username)
            ...
            logger.error("Get locked accounts error", error=str(e), error_type=type(e).__name__)
            return {
                "success": False,
    ...
    async def update_account_lock(self, params: Dict[str, Any], ctx: Context = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check admin role
            if user.role.value != "admin":
                logger.warning("Non-admin user attempted to update account lock",
                             username=user.username, role=user.role.value, lock_id=lock_id)
                return {
                    "success": False,
                    "message": "Admin privileges required",
                    "error": "ADMIN_REQUIRED"
            ...
            await self.auth_service._log_security_event(
                ...
                success=True,
                client_ip=ctx.meta.get('clientIp', 'unknown') if ctx and hasattr(ctx, 'meta') else 'unknown',
                user_agent=ctx.meta.get('userAgent', 'unknown') if ctx and hasattr(ctx, 'meta') else 'unknown'
            )

            logger.info(f"Account {action}",
                       lock_id=lock_id,
            ...
            logger.error("Update account lock error", error=str(e), error_type=type(e).__name__)
            return {
                "success": False,
