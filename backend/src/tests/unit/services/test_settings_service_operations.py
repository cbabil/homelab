...
from models.settings import (
    SettingCategory, SettingScope, SettingDataType, SettingValue,
    SettingsRequest, SettingsUpdateRequest, SettingsValidationResult, SystemSetting,
)
...
class TestGetSettings:
    ...
    async def test_get_settings_success(self, settings_service, mock_connection):
        ...
        mock_cursor.fetchall = AsyncMock(return_value=[{
            "setting_key": "ui.theme", "setting_value": '"dark"', "category": "ui",
            "scope": "user_overridable", "data_type": "string", "is_admin_only": 0,
        }])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
            user_id="user-123", include_system_defaults=True, include_user_overrides=False,
        )
        with patch("services.settings_service.logger"):
    ...
    @pytest.mark.asyncio
    async def test_get_settings_with_category_filter(self, settings_service, mock_connection):
        """get_settings should filter by category."""
        mock_cursor = AsyncMock()
        mock_cursor.fetchall = AsyncMock(return_value=[])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
            user_id="user-123", category=SettingCategory.UI,
            include_system_defaults=True, include_user_overrides=False,
        )
        with patch("services.settings_service.logger"):
            result = await settings_service.get_settings(request)
        assert result.success is True
    ...
    @pytest.mark.asyncio
    async def test_get_settings_with_specific_keys(self, settings_service, mock_connection):
        """get_settings should filter by specific keys."""
        mock_cursor = AsyncMock()
        mock_cursor.fetchall = AsyncMock(return_value=[])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
            user_id="user-123", setting_keys=["ui.theme", "ui.language"],
            include_system_defaults=True, include_user_overrides=False,
        )
        with patch("services.settings_service.logger"):
            result = await settings_service.get_settings(request)
    ...
    async def test_get_settings_admin_only_excluded_for_regular_user(
        ...
        mock_cursor = AsyncMock()
        mock_cursor.fetchall = AsyncMock(return_value=[{
            "setting_key": "security.admin_setting", "setting_value": '"secret"',
            "category": "security", "scope": "system", "data_type": "string",
            "is_admin_only": 1,
        }])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        mock_db_service.get_user_by_id = AsyncMock(return_value=regular_user)
        request = SettingsRequest(
            user_id="user-456", include_system_defaults=True, include_user_overrides=False,
        )
        with patch("services.settings_service.logger"):
            result = await settings_service.get_settings(request)
        assert result.success is True
    ...
    @pytest.mark.asyncio
    async def test_get_settings_includes_user_overrides(self, settings_service, mock_connection):
        """get_settings should include user overrides."""
        mock_cursor = AsyncMock()
        mock_cursor.fetchall = AsyncMock(side_effect=[
            [{"setting_key": "ui.theme", "setting_value": '"light"', "category": "ui",
              "scope": "user_overridable", "data_type": "string", "is_admin_only": 0}],
            [{"setting_key": "ui.theme", "setting_value": '"dark"',
              "category": "ui", "data_type": "string"}],
        ])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
            user_id="user-123", include_system_defaults=True, include_user_overrides=True,
        )
        with patch("services.settings_service.logger"):
            result = await settings_service.get_settings(request)
        assert result.success is True
    ...
    async def test_get_settings_handles_invalid_system_setting_value(
        ...
        mock_cursor.fetchall = AsyncMock(return_value=[{
            "setting_key": "ui.theme", "setting_value": "not valid json",
            "category": "ui", "scope": "user_overridable",
            "data_type": "string", "is_admin_only": 0,
        }])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
            user_id="user-123", include_system_defaults=True, include_user_overrides=False,
    ...
    async def test_get_settings_handles_invalid_user_setting_value(
        ...
        mock_cursor.fetchall = AsyncMock(side_effect=[
            ...
            [{"setting_key": "ui.theme", "setting_value": "invalid json",
              "category": "ui", "data_type": "string"}],
        ])
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsRequest(
...
class TestValidateSettings:
    ...
    async def test_validate_settings_unknown_setting(self, settings_service):
        ...
        with (
            ...
            result = await settings_service.validate_settings({"unknown.key": "value"}, "user-123")
    ...
    async def test_validate_settings_admin_only_flagged(
        ...
        mock_validated = SettingValue(raw_value='"dark"', data_type=SettingDataType.STRING)

        with (
            patch("services.settings_service.logger"),
            patch.object(settings_service, "get_system_setting",
                        new_callable=AsyncMock, return_value=sample_system_setting),
            patch.object(settings_service, "_validate_setting_value",
                        new_callable=AsyncMock, return_value=mock_validated),
        ):
            result = await settings_service.validate_settings({"ui.theme": "dark"}, "user-123")
        assert result.is_valid is True
        assert "ui.theme" in result.admin_required

    @pytest.mark.asyncio
    async def test_validate_settings_validation_error(self, settings_service, sample_system_setting):
        ...
        with (
            ...
            result = await settings_service.validate_settings({"ui.theme": "bad"}, "user-123")
        assert result.is_valid is False
    ...
    async def test_validate_settings_security_violation(
        ...
        with (
            ...
            patch.object(settings_service, "get_system_setting",
                        new_callable=AsyncMock, return_value=sample_system_setting),
            patch.object(settings_service, "_validate_setting_value",
                        new_callable=AsyncMock, side_effect=RuntimeError("Security check failed")),
        ):
            result = await settings_service.validate_settings({"ui.theme": "evil"}, "user-123")
        assert result.is_valid is False
        assert any("Security violation" in e for e in result.security_violations)

    @pytest.mark.asyncio
    async def test_validate_settings_success(self, settings_service, sample_system_setting):
    ...
    @pytest.mark.asyncio
    async def test_validate_settings_handles_exception_gracefully(self, settings_service):
        ...
        with (
            ...
            result = await settings_service.validate_settings({"ui.theme": "value"}, "user-123")
...
class TestUpdateSettings:
    ...
    async def test_update_settings_success(self, settings_service, mock_connection):
        ...
        valid_result = SettingsValidationResult(is_valid=True, validated_settings={"ui.theme": "dark"})
        mock_cursor = AsyncMock()
        mock_cursor.lastrowid = 1
        mock_connection.execute = AsyncMock(return_value=mock_cursor)
        request = SettingsUpdateRequest(user_id="user-123", settings={"ui.theme": "dark"})
        with (
            patch("services.settings_service.logger"),
            patch.object(settings_service, "validate_settings",
                        new_callable=AsyncMock, return_value=valid_result),
            patch.object(settings_service, "_update_single_setting",
    ...
    async def test_update_settings_validation_failed(self, settings_service):
        ...
        invalid_result = SettingsValidationResult(
            is_valid=False, errors=["Unknown setting: invalid.key"], security_violations=[],
        )
        request = SettingsUpdateRequest(user_id="user-123", settings={"invalid.key": "value"})
        with (
            patch("services.settings_service.logger"),
            patch.object(settings_service, "validate_settings",
                        new_callable=AsyncMock, return_value=invalid_result),
        ):
            result = await settings_service.update_settings(request)
        assert result.success is False
    ...
    async def test_update_settings_admin_required(
        ...
        valid_result = SettingsValidationResult(
            is_valid=True, validated_settings={"security.setting": "value"},
            admin_required=["security.setting"],
        )
        mock_db_service.get_user_by_id = AsyncMock(return_value=regular_user)
        request = SettingsUpdateRequest(user_id="user-456", settings={"security.setting": "value"})
        with (
            patch("services.settings_service.logger"),
            patch.object(settings_service, "validate_settings",
                        new_callable=AsyncMock, return_value=valid_result),
    ...
    @pytest.mark.asyncio
    async def test_update_settings_rollback_on_error(self, settings_service, mock_connection):
        ...
        with (
            ...
            patch.object(settings_service, "validate_settings",
                        new_callable=AsyncMock, return_value=valid_result),
            patch.object(settings_service, "_update_single_setting",
                        new_callable=AsyncMock, side_effect=Exception("Update error")),
        ):
            result = await settings_service.update_settings(request)
        assert result.success is False
    ...
    @pytest.mark.asyncio
    async def test_update_settings_has_checksum(self, settings_service, mock_connection):
