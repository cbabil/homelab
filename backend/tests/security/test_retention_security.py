...
from models.retention import (
    DataRetentionSettings, CleanupRequest, SecurityValidationResult,
...
class TestPrivilegeEscalation:
    ...
    async def test_regular_user_cannot_update_retention_settings(
        ...
        settings = DataRetentionSettings(log_retention_days=7)  # Dangerous short retention
...
class TestInputValidationAndSanitization:
    ...
    async def test_sql_injection_prevention_in_user_id(self, retention_tools):
        """Test prevention of SQL injection via user ID."""
        malicious_user_ids = [
            "'; DROP TABLE users; --",
        ...
        for malicious_id in malicious_user_ids:
            result = await retention_tools.get_retention_settings(user_id=malicious_id)
            # Should either fail validation or handle safely
            assert result is not None

    async def test_xss_prevention_in_input_fields(self, retention_tools):
        ...
        for payload in xss_payloads:
            ...
            # Should fail validation due to unknown field or be sanitized
            assert result["success"] is False or "custom_field" not in result.get("data", {})

    async def test_command_injection_prevention(self, retention_tools):
        ...
        for payload in command_injection_payloads:
            request_data = {
                "retention_type": "logs",
                "admin_user_id": f"admin{payload}",
                "session_token": f"token{payload}"
            }

            result = await retention_tools.preview_cleanup(request_data)
            # Should fail validation
            assert result["success"] is False

    async def test_path_traversal_prevention(self, retention_tools):
        ...
        for payload in path_traversal_payloads:
            # Try path traversal in various fields
            settings_data = {
                "log_retention_days": 30,
    ...
    async def test_buffer_overflow_prevention(self, retention_tools):
        ...
        oversized_data = {
            "retention_type": "logs",
            "admin_user_id": oversized_string,
            "session_token": oversized_string
        }

        result = await retention_tools.preview_cleanup(oversized_data)
        # Should fail validation or be handled safely
        assert result["success"] is False

    async def test_unicode_exploitation_prevention(self, retention_tools):
        ...
        for payload in unicode_payloads:
            request_data = {
                "retention_type": "logs",
                "admin_user_id": f"admin{payload}test",
                "session_token": "valid-token"
            }
...
class TestDataAccessControlAndLeaks:
    ...
    async def test_user_cannot_access_other_user_settings(self, retention_tools):
        """Test users cannot access other users' retention settings."""
        # Mock service to return settings for any user ID
        mock_settings = DataRetentionSettings(log_retention_days=60)
        retention_tools.retention_service.get_retention_settings.return_value = mock_settings

        # User A tries to access User B's settings
        result = await retention_tools.get_retention_settings(user_id="other-user-123")

        # The service should validate user permissions before returning settings
        assert result is not None
        # In a real implementation, this should check if the requesting user
        # has permission to access the specified user's settings

    async def test_error_messages_do_not_leak_sensitive_info(self, retention_tools):
        """Test error messages don't leak sensitive information."""
        # Force various error conditions
        retention_tools.retention_service.get_retention_settings.side_effect = Exception(
            "Database connection failed: server=db.internal.company.com, user=admin, password=secret123"
        )

        result = await retention_tools.get_retention_settings(user_id="test-user")

        assert result["success"] is False
        # Error message should not contain sensitive database details
        assert "password=secret123" not in result["message"]
        assert "db.internal.company.com" not in result["message"]

    async def test_user_enumeration_prevention(self, retention_tools):
        ...
        result1 = await retention_tools.get_retention_settings(user_id="nonexistent-user")
        result2 = await retention_tools.get_retention_settings(user_id="another-user")
    ...
    async def test_information_disclosure_in_preview_results(self, retention_tools):
...
class TestRateLimitingAndDoSPrevention:
    ...
    async def test_large_payload_rejection(self, retention_tools):
        """Test rejection of oversized payloads."""
        # Create oversized settings data
        oversized_settings = {
            "log_retention_days": 30,
        ...
        result = await retention_tools.update_retention_settings(
            settings_data=oversized_settings,
            user_id="admin-test"
        )
    ...
    async def test_nested_object_depth_limit(self, retention_tools):
    ...
    async def test_array_size_limit(self, retention_tools):
        """Test protection against large array attacks."""
        large_array_settings = {
            "log_retention_days": 30,
            "large_array": ["item"] * 100000  # Large array
        }

        result = await retention_tools.update_retention_settings(
            settings_data=large_array_settings,
            user_id="admin-test"
        )
...
class TestConcurrencyAndRaceConditions:
    ...
    async def test_concurrent_privilege_escalation_prevention(self, retention_service):
        """Test prevention of privilege escalation through race conditions."""
        # Mock scenario where user role changes during operation
        user_states = [
            User(id="test-user", username="test", email="test@example.com", role=UserRole.USER, last_login="2023-09-14T09:30:00.000Z", is_active=True, preferences={}),
            User(id="test-user", username="test", email="test@example.com", role=UserRole.ADMIN, last_login="2023-09-14T10:30:00.000Z", is_active=True, preferences={})
        ]
        ...
        def mock_get_user(username):
            nonlocal call_count
            result = user_states[call_count % len(user_states)]
            call_count += 1
            return result

        retention_service.auth_service._validate_jwt_token.return_value = {"username": "test"}
        retention_service.auth_service.get_user_by_username.side_effect = mock_get_user
        ...
        # Security validation should be consistent
        # Either all succeed (if user is admin) or all fail (if user is not admin)
        success_count = sum(1 for v in validations if v.is_valid)
        assert success_count == 0 or success_count == 10
...
class TestAuditBypassAndTampering:
    ...
    async def test_audit_tampering_prevention(self, retention_service):
        ...
        # This test ensures that audit log creation is separate from main operation
        # and cannot be affected by main operation failures

        with patch('services.retention_service.log_service') as mock_log_service:
            # Mock audit service to track calls
            audit_calls = []

            def track_audit_call(log_entry):
                audit_calls.append(log_entry)

            mock_log_service.create_log_entry.side_effect = track_audit_call

            # Force main operation to fail
            ...
            # But audit should still be recorded
            assert len(audit_calls) > 0
            assert audit_calls[-1].level == "ERROR"
...
class TestSecurityConfigurationValidation:
    ...
    async def test_minimum_retention_periods_enforced(self):
        ...
        with pytest.raises(Exception):  # Should raise validation error
            DataRetentionSettings(
                log_retention_days=1,  # Too short - could hide attacks
    ...
    async def test_maximum_retention_periods_enforced(self):
        ...
        with pytest.raises(Exception):  # Should raise validation error
            DataRetentionSettings(
                log_retention_days=10000,  # Excessive retention
                user_data_retention_days=50000  # Excessive retention
            )

    async def test_audit_log_compliance_retention_enforced(self):
        """Test that audit logs meet compliance retention requirements."""
        with pytest.raises(Exception):  # Should raise validation error
            DataRetentionSettings(
                audit_log_retention_days=180  # Below 1 year compliance requirement
            )

    async def test_dangerous_auto_cleanup_validation(self):
        """Test validation of dangerous auto-cleanup configurations."""
        # This should be flagged as dangerous
        settings = DataRetentionSettings(
            log_retention_days=7,  # Minimum allowed but dangerous
            auto_cleanup_enabled=True  # Auto-cleanup with short retention
        )

        # The system should allow this but flag it as requiring extra caution
