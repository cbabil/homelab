...
export function useSettingsState() {
  ...
  const saveWithIndicator = useCallback(async (
    section: 'notifications' | 'servers' | 'agent',
  ...
  const setServerAlerts = useCallback((checked: boolean) => {
    saveWithIndicator('notifications', { serverAlerts: checked })
  }, [saveWithIndicator])

  const setResourceAlerts = useCallback((checked: boolean) => {
    saveWithIndicator('notifications', { resourceAlerts: checked })
  }, [saveWithIndicator])

  const setUpdateAlerts = useCallback((checked: boolean) => {
    saveWithIndicator('notifications', { updateAlerts: checked })
  }, [saveWithIndicator])

  const setConnectionTimeout = useCallback((timeout: string) => {
    saveWithIndicator('servers', { connectionTimeout: parseInt(timeout, 10) })
  }, [saveWithIndicator])

  const setRetryCount = useCallback((count: string) => {
  ...
  const setPreferAgent = useCallback((checked: boolean) => {
    saveWithIndicator('agent', { preferAgent: checked })
  }, [saveWithIndicator])

  const setAgentAutoUpdate = useCallback((checked: boolean) => {
    saveWithIndicator('agent', { autoUpdate: checked })
  }, [saveWithIndicator])

  const setHeartbeatInterval = useCallback((interval: string) => {
    saveWithIndicator('agent', { heartbeatInterval: parseInt(interval, 10) })
  }, [saveWithIndicator])

  const setHeartbeatTimeout = useCallback((timeout: string) => {
    saveWithIndicator('agent', { heartbeatTimeout: parseInt(timeout, 10) })
  }, [saveWithIndicator])

  const setCommandTimeout = useCallback((timeout: string) => {
