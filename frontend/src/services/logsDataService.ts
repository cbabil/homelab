...
import {
  ServiceResponse,
  DataServiceOptions,
  BaseDataService as IBaseDataService,
...
export class LogsDataService
  ...
  implements IBaseDataService<LogEntry, LogFilterOptions>
{
  ...
  private mapDatabaseLogToFrontend(dbLog: DatabaseLogEntry): LogEntry {
    const tags = this.parseTags(dbLog.tags);

    // Determine category from source or tags
    let category: LogEntry['category'] = 'system';
    if (dbLog.source === 'docker' || tags.includes('docker')) {
      category = 'application';
    } else if (dbLog.source === 'application' || tags.includes('application')) {
      category = 'application';
    } else if (tags.includes('security') || dbLog.source.includes('ssh')) {
      category = 'security';
    ...
    return {
      ...
      details: tags.length > 0 ? `Tags: ${tags.join(', ')}` : undefined,
    };
  }
  ...
  async getAll(filter?: LogFilterOptions): Promise<ServiceResponse<LogEntry[]>> {
    ...
    if (toolResponse?.success && toolResponse.data?.logs) {
      const mappedLogs = toolResponse.data.logs.map((log: DatabaseLogEntry) =>
        this.mapDatabaseLogToFrontend(log)
      );
      return {
        success: true,
        data: mappedLogs,
