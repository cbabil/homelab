...
class SessionService {
  ...
  async destroySession(): Promise<void> {
    ...
    try {
      ...
      if (accessToken) {
        revocationPromises.push(jwtService.revokeToken(accessToken, 'logout').catch(console.error));
      }
  ...
  private async clearSessionMetadata(): Promise<void> {
    try {
      const currentSessionId =
        this.currentSession?.sessionId || localStorage.getItem('current_session_id');
